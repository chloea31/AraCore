...
def save_fba_to_excel(c3_model, c4_model, D_fba, D_exp, theNotebook, file_name='pfba_results'):
    ...
    for exp in D_exp:
        ...
        for r_obj in c3_model.reactions:
            ...
            if r_obj.lower_bound == 0:
                r_lb = '0' 
            else:
                if r_obj.lower_bound == -inf:
                    r_lb = '-inf' 
                else:
                    r_lb = str(r_obj.lower_bound)
            if r_obj.upper_bound == 0:
                r_ub = '0' 
            else:
                if r_obj.upper_bound == inf:
                    r_ub = 'inf' 
                else:
                    r_ub = str(r_obj.lower_bound) 
        ...
        for r_id in L_r_transport:
            ...
            if r_obj.lower_bound == 0:
                ...
                if r_obj.lower_bound == -inf:
                    ...
                    r_lb = str(r_obj.lower_bound)
            # 1 assignation on r_lb; its value depends on several cases (3 possible values according to the case
...
def save_fva_to_excel(c3_model, c4_model, D_fva, D_exp, theNotebook, file_name='fva_results'):
    ...
    for exp in D_exp:
        ...
        for r_obj in c3_model.reactions:
            r_id = r_obj.id
            r_M_max = D_fva[exp].loc['[M]_'+r_id,'maximum']
            r_M_min = D_fva[exp].loc['[M]_'+r_id,'minimum']
            r_B_max = D_fva[exp].loc['[B]_'+r_id,'maximum']
            r_B_min = D_fva[exp].loc['[B]_'+r_id,'minimum']
        ...
        for r_id in L_r_transport:
            r_obj = c4_model.reactions.get_by_id(r_id)
            r_MB_max = D_fva[exp].loc[r_id,'maximum']
            r_MB_min = D_fva[exp].loc[r_id,'minimum']
