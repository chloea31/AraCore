...
def plot_transport_MB(D_fba, D_exp, L_r, xaxis_title, save_fig, cut_off):
    ...
    for entry in D_rid_meanFlux_MB:
        ...
        for x in D_exp:
            if D_fba[x][r_id] > 0:
                df.loc[r_id, D_exp[x]] = abs(D_fba[x][r_id]) if not abs(D_fba[x][r_id]) < cut_off else float('nan')
                # .loc[] to locate a specific value in a dataframe
                # always takes the index AND the column => 2 elements
                # if we want to assign this value, once we located it, we assign it
    ...
    layout_MB = go.Layout(width = 500,
                          margin = {'b':100},
                          yaxis = {'tickmode': 'array', 'tickvals': list(range(0,len(yTickNames_MB))), 'ticktext': yTickNames_MB, 'title': 'Transport Metabolites'},
                          xaxis= {'title':xaxis_title,'tickangle':45 if len(D_exp) >= 8 else 0},
...
def plot_transport_BM(D_fba, D_exp, L_r, xaxis_title, save_fig, cut_off):
    ...
    layout_BM = go.Layout(width = 500,
                          margin = {'b':100},
                          yaxis = {'tickmode': 'array', 'tickvals': list(range(0,len(yTickNames_BM))), 'ticktext': yTickNames_BM, 'title': 'Transport Metabolites'},
                          xaxis= {'title':xaxis_title,'tickangle':45 if len(D_exp) >= 8 else 0},
    ...
    if len(D_exp) > len(L_colors):
        ...
        for exp in D_exp:
            ...
            for r_id in L_r_org:
                ...
                D_FVA['max'] = D_pfva[exp].loc[r_id,'maximum']
                D_FVA['min'] = D_pfva[exp].loc[r_id,'minimum']
